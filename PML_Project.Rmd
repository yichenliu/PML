...
### 1. Synopsis
In this study, we utilise Random Forest algorithm, with 4 folds cross validation method, to predict the quality of a series of activities conducted by the experiment participants. The model yields a cross validation error of 0.62%, and the out of sample error for our model is 0.53%. This aligns with our expectation that the out of sample error should be similar to the cross validation error. Lastly, we use a testing dataset to predict the quality of corresponding activities and submit the results to Coursera to validate the model.

### 2. Data Processing
This study will use the training dataset, which is extracted from [here](https://d396qusza40orc.cloudfront.net/predmachlearn/pml-training.csv), to specify our acitivity quality prediction model. After downloadning the dataset, we will first load the dataset into the data frame `pml_training`.
```{r, cache = TRUE, warning = FALSE, message = FALSE}
pml_training <- read.csv("pml-training.csv", na.strings = c("", "NA"))
```

Columns that contain missing values and unuseful information (such as: user name and time, etc.), will then be removed as followed.
```{r, cache = TRUE, warning = FALSE, message = FALSE}
...
The training dataset contains 53 variables, with the last column containing `classe` variable we are trying to predict. We begin by looking at the correlations between the variables in our dataset. As plot shows that only few variables shows strong correlation, we will use all variables to predict `classe`.
...
**As shown below, the final model has a cross validation error of 0.62%. We expect that the out of sample error should be similar to the cross validation error.**
```{r, cache = TRUE, warning = FALSE, message = FALSE}
modFit$finalModel
```

Next, we assess the performance of our model using the out of sample dataset that we reserved before (25% of the observations).
...
Lastly, we use a testing dataset, which is extracted from [here](https://d396qusza40orc.cloudfront.net/predmachlearn/pml-testing.csv), to predict the quality of corresponding activities and submit the results to Coursera to validate the model. To do this, we will first process the testing dataset in a way that is same the way we process the training dataset.
```{r, cache = TRUE, warning = FALSE, message = FALSE}
...
Next we will apply the model that we estimated using training dataset to predict `classe` using the testing dataset, and results are shown as followed.
```{r, cache = TRUE, warning = FALSE, message = FALSE}
